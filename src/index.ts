...
class LsDocMakerAxiosMiddlewareBuildOptions {
    ...
    constructor(
        itemsPaths: Array<string>,
        outputPath: string,
        title: string,
        url: string,
        observation: string,
        clearJsonFilesAfterBuild: boolean = false
    ) {
...
function buildLsDocMaker(options: LsDocMakerAxiosMiddlewareBuildOptions): Doc {
    if (!options._itemsPaths || options._itemsPaths.length <= 0)
        throw new Error('At least one item path must be provided')
    ...
    forEach(options._itemsPaths, itemsPath => {
        ...
        readdirSync(`${itemsPath}\\${__ls_path__}\\`).forEach(file => {
            let items: any = readFileSync(
                `${itemsPath}\\${__ls_path__}\\${file}`
            ).toString()
